...
sudo nixos-rebuild switch --flake ~/dotfiles#nixos &>nixos-switch.log || (cat nixos-switch.log | grep --color error && exit 1)
home-manager switch --flake ~/dotfiles#brightonlcox &>nixos-switch.log || (cat nixos-switch.log | grep --color error && exit 1)
